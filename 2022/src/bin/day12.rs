...
fn dijkstra(elevation_map: &Vec<Vec<u32>>, start: &Position, end: &Position) -> usize {
        ...
        for d in [
            ...
            if can_move(elevation_map, &current_pos, &possible) {
                if let Some(v) = q.get_mut(&possible) {
                    if !v.visited && v.distance > u.distance+1 {
                        v.distance = u.distance + 1;
                    }
                }
            }
        }
        if current_pos == start.to_owned() { 
...
fn main() {
    ...
    let mut x = 0;
    let mut y = 0;
    let mut starting = Position::new(0, 0);
    ...
    println!("Steps: {}", dijkstra(&elevation_map, &starting, &ending));
}
